...
fn part2(input: &str) -> Result<String, String> {
    ...
    for (p1, p2, _) in nearests.iter() {
        ...
        while i < circuits.len() {
            ...
            // found the last connection to make all junctions boxes belong
            // to the same circuit
            if circuits.len() == 1 && circuits[0].len() == coords.len() {
                return Ok((coords[*p1][0] * coords[*p2][0]).to_string());
            }
        }
    ...
    Err::<String, String>("Couldn't connect all junction boxes on the same circuit".to_owned())
}
